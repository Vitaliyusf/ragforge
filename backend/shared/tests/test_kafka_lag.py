...
def gauge_value(*, service: str, topic: str, group: str) -> float:
    ...
    return METRICS.kafka_consumer_lag.labels(
...
def test_lag_is_the_distance_between_the_two_next_offsets():
...
def test_a_caught_up_consumer_records_a_genuine_zero():
...
def test_an_unknown_offset_records_nothing(highwater, next_offset):
...
def test_a_negative_difference_is_clamped_to_zero():
...
def test_each_topic_and_group_is_its_own_series():
...
def kafka_base(monkeypatch):
    ...
    structs.TopicPartition = collections.namedtuple("TopicPartition", ["topic", "partition"])
...
def build_consumer(kafka_base, topic: str, group: str, *, records, highwater):
...
def test_consuming_a_message_records_the_lag(kafka_base):
...
def test_the_last_message_of_a_caught_up_partition_reads_zero(kafka_base):
    consumer = build_consumer(
...
def test_an_unknown_highwater_writes_no_series_and_still_consumes(kafka_base):
...
def test_a_failing_gauge_never_breaks_consumption(kafka_base):
...
def test_the_config_service_name_labels_the_series(kafka_base):
...
def test_a_config_without_a_service_name_mislabels_rather_than_crashing(kafka_base):
...
def test_lag_uses_the_message_topic_not_the_subscription(kafka_base):
