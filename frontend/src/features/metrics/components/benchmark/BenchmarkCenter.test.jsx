...
describe('BenchmarkCenter', () => {
  ...
  it.each(['partial', 'failed'])('shows a terminal %s benchmark and allows its archive to download', async (status) => {
    ...
    expect(screen.getByText('1 completed, 0 partial of 4 executable phases')).toBeInTheDocument()
  ...
  it('shows live phase progress and prevents another full run', () => {
    setup({ benchmark_id: 'b-1', status: 'running', progress: { completed_phases: 1, total_phases: 4 }, phases: [{ name: 'retrieval_base', status: 'completed', results: { mrr: 0.8, mean_latency_ms: 11 } }, { name: 'end_to_end_regular', status: 'running' }] })
